...
const SCRUB_RULES: Array<{ label: string; pattern: RegExp }> = [
  // Email addresses
  {
    label: "[EMAIL]",
  ...
  // US Social Security Number (SSN) - 000-00-0000
  {
    label: "[SSN]",
    pattern:
      /\b(?!000|666|9\d{2})([0-8]\d{2}|7([0-6]\d))([- ]?)(?!00)\d{2}\3(?!0000)\d{4}\b/g,
  },
  ...
  // Full names (Title + Capitalized words — conservative pattern)
  {
    label: "[NAME]",
    pattern:
      /\b(?:Mr|Mrs|Miss|Ms|Dr|Prof|Sir|Madam)\.?\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*/g,
  },
