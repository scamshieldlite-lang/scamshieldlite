...
import { nowPlusDays } from "../utils/date.js";
import { logger } from "../utils/logger.js";
export const subscriptionService = {
    // ── Create trial ─────────────────────────────────────────────
    async createTrialSubscription(userId, trialDays) {
    ...
    async hasFullAccess(userId) {
        return ((await this.isTrialActive(userId)) || (await this.isPaidActive(userId)));
    },
    // ── Purchase verification ─────────────────────────────────────
    async verifyAndActivate(userId, purchaseToken, productId) {
        // 1. Verify with Google Play
        const purchaseInfo = await googlePlayService.verifySubscription(purchaseToken, productId);
        if (!purchaseInfo) {
        ...
        logger.info({ userId, productId, currentPeriodEnd }, "Subscription activated");
        return this.getSubscriptionState(userId);
    },
    // ── Webhook handlers ──────────────────────────────────────────
    async handleRenewal(userId, purchaseToken, productId, expiryTimeMillis) {
    ...
    buildState(sub) {
        ...
        if (sub.currentPeriodEnd && sub.currentPeriodEnd > now) {
            daysRemaining = Math.ceil((sub.currentPeriodEnd.getTime() - now.getTime()) /
                (1000 * 60 * 60 * 24));
        }
        // 2. Fall back to Trial if no paid period exists
        else if (isTrialActive) {
            daysRemaining = Math.ceil((sub.trialEnd.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        }
        const plan = isPaidActive
            ? "paid"
            : isTrialActive
                ? "free_trial"
                : "expired";
