...
class GooglePlayService {
    ...
    async verifySubscription(purchaseToken, productId) {
        try {
            ...
            const isPaymentReceived = purchase.paymentState === 1 || purchase.paymentState === 2;
            if (!isPaymentReceived) {
                logger.warn({ productId, paymentState: purchase.paymentState }, "Play purchase token has unpaid state");
                return null;
            }
            return {
            ...
            logger.error({ error, productId }, "Google Play purchase verification failed");
            return null;
        }
    }
    /**
