...
export const reportController = {
    async submit(req, res, next) {
        try {
            ...
            if (!parsed.success) {
                throw new InvalidInputError(parsed.error.issues[0]?.message ?? "Invalid report data");
            }
            // ── Extract identity ──────────────────────────────────────
            ...
            if (!req.user?.id && !deviceFingerprint) {
                throw new InvalidInputError("Device fingerprint is required for guest reports");
            }
            // ── Submit ────────────────────────────────────────────────
            ...
            logger.info({ requestId: req.id, reportId, isDuplicate }, "Report submitted");
