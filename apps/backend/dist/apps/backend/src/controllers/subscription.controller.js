...
export const subscriptionController = {
    ...
    async getStatus(req, res, next) {
        try {
            ...
            const subscription = await subscriptionService.getSubscriptionState(req.user.id);
    ...
    async ensureTrial(req, res, next) {
        try {
            if (!req.user)
                throw new UnauthorizedError();
            const existing = await subscriptionService.getSubscription(req.user.id);
            if (!existing) {
                await subscriptionService.createTrialSubscription(req.user.id, env.TRIAL_DURATION_DAYS);
                logger.info({ userId: req.user.id }, "Trial subscription created via ensure-trial");
            }
            const existingConsent = await consentService.getLatestConsent(req.user.id);
            if (!existingConsent) {
                ...
                logger.info({ userId: req.user.id }, "Consent recorded via ensure-trial fallback");
            }
            const subscription = await subscriptionService.getSubscriptionState(req.user.id);
    ...
    async verifyPurchase(req, res, next) {
        try {
            ...
            if (!parsed.success) {
                throw new InvalidInputError(parsed.error.issues[0]?.message ?? "Invalid request");
            }
            ...
            const subscription = await subscriptionService.verifyAndActivate(req.user.id, purchaseToken, productId);
            const response = {
                success: true,
                subscription,
                message: "Subscription activated successfully",
            };
            logger.info({ userId: req.user.id, productId }, "Purchase verified and subscription activated");
    ...
    async handleWebhook(req, res, next) {
        try {
            ...
            logger.info({ notificationType, subscriptionId }, "RTDN webhook received");
            // Find the user by purchase token
            const sub = await db.query.subscriptions.findFirst({
                where: eq(subscriptions.playPurchaseToken, purchaseToken),
            });
            if (!sub) {
                logger.warn({ purchaseToken: purchaseToken.substring(0, 20) }, "RTDN webhook: no subscription found for token");
                res.status(200).json({ received: true });
                return;
            }
            // Notification types:
            ...
            switch (notificationType) {
                ...
                case 1: // RECOVERED
                    ...
                    const purchaseInfo = await import("../services/googlePlay.service.js").then((m) => m.googlePlayService.verifySubscription(purchaseToken, subscriptionId));
                    if (purchaseInfo) {
                        await subscriptionService.handleRenewal(sub.userId, purchaseToken, subscriptionId, purchaseInfo.expiryTimeMillis);
                    }
                    break;
                case 3: // CANCELED
                case 12: // EXPIRED
