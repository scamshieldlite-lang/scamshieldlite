import { consentService, CURRENT_PRIVACY_VERSION, CURRENT_TERMS_VERSION, } from "../services/consent.service.js";
...
export const privacyController = {
    ...
    async recordConsent(req, res, next) {
        try {
            ...
            await consentService.recordConsent(req.user.id, ip, req.headers["user-agent"]);
     ...
     */
    async deleteScanHistory(req, res, next) {
        try {
            if (!req.user)
                throw new UnauthorizedError();
