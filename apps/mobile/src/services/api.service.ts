...
const PRIMARY_URL = process.env.EXPO_PUBLIC_API_BASE_URL; // Railway
const FALLBACK_URL = process.env.EXPO_PUBLIC_FALLBACK_API_URL; // Render
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Check if network error or server error (5xx) AND we haven't already retried
    const isNetworkOrServerError =
      !error.response ||
      (error.response.status >= 500 && error.response.status < 600);

    if (isNetworkOrServerError && !originalRequest._retry && FALLBACK_URL) {
      originalRequest._retry = true;

      // Switch base URL to Render
      originalRequest.baseURL = FALLBACK_URL;

      console.log(
        `[Failover] Primary failed. Retrying request with Fallback: ${FALLBACK_URL}`,
      );

      // Re-run the request with the secondary URL
